...
def main():
    ...
    with open("data-sets/1-2-0-td-set.json", "w") as file:
        file.write(torsion_set.json())

    select_parameters(
        torsion_set,
        parameter_types=["ProperTorsions"],
        output_path="data-sets/1-2-0-td-set-torsion-smirks.json",
    )
    ...
    with open("data-sets/1-2-0-opt-set.json", "w") as file:
        file.write(optimization_set.json())

    select_parameters(
        optimization_set,
        parameter_types=["Bonds", "Angles"],
        output_path="data-sets/1-2-0-opt-set-valence-smirks.json",
    )

    hessian_set = optimization_set.to_basic_result_collection(driver="hessian")

    with open("data-sets/1-2-0-hess-set.json", "w") as file:
